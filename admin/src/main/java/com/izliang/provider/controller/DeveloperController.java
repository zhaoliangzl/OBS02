...
public class DeveloperController {
    ...
    @ApiImplicitParams(value = {
            @ApiImplicitParam(paramType = "query",dataType = "int",name = "id",value = "信息id",required = true),
            @ApiImplicitParam(paramType = "query",dataType = "long",name = "totalFlow",value = "当月流量总计",required = true),
            @ApiImplicitParam(paramType = "query",dataType = "long",name = "totalSize",value = "用户存储大小",required = true),
            @ApiImplicitParam(paramType = "query",dataType = "long",name = "useFlow",value = "当月使用流量",required = true),
            @ApiImplicitParam(paramType = "query",dataType = "long",name = "useSize",value = "用户已经使用的存储大小",required = true),
    })
    public JSONObject update(
        ...
        System.out.println("id = [" + id + "], totalFlow = [" + totalFlow + "], totalSize = [" + totalSize + "], useFlow = [" + useFlow + "], useSize = [" + useSize + "]");
        Developer developer = developerRepository.getOne(id);
        if(developer !=null){
            developer.setUseSize(useSize);
            developer.setTotalSize(totalSize);
            developer.setTotalFlow(totalFlow);
            developer.setUseFlow(useFlow);
            ...
            return Result.Result(404,"该用户不存在","");
        }
